    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int result = stock1.get("name").toString().compareToIgnoreCase(stock2.get("name").toString());
                if(result!= 0){
                    return result;
                }else {
                    SimpleDateFormat sf = new SimpleDateFormat("yyyyMMdd");
                    result = sf.format((Date)stock1.get("date")).compareTo(sf.format((Date)stock2.get("date")));
                    if(result!=0){
                        return (-result);
                    }else{
                        double dif = !stock1.containsKey("open")?(double)stock1.get("change")
                                :(double)stock1.get("last")-(double)stock1.get("open");
                        double dif2 = !stock2.containsKey("open")?(double)stock2.get("change")
                                :(double)stock2.get("last")-(double)stock2.get("open");
                        return (-Double.compare(dif,dif2));
                    }
                }
            }
        });
